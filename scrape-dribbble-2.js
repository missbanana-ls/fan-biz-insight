...
const SEARCHES = [
  { q: 'e-commerce promotion sale discount mobile app', tag: '电商大促' },
  { q: 'daily check in streak reward mobile app', tag: '签到活动' },
  { q: 'referral invite friend bonus mobile', tag: '社交裂变' },
  { q: 'subscription pricing plan paywall mobile', tag: '订阅付费' },
  { q: 'loyalty program points reward mobile', tag: '积分活动' },
];

// ALL existing image URLs to deduplicate (including the 5 we just found)
...
(async () => {
  ...
  const fs = require('fs');
  fs.writeFileSync('scraped-results-2.json', JSON.stringify(allResults, null, 2));
